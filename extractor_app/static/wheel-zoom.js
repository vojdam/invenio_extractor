(function (global, factory) {
    ...
    switch (align) {
      ...
      default:
        if (
          ...
          shift =
            ((size - originalViewportSize) / 2 + correctCoordinate) * positive;
        }
    }
    return shift;
  }
  ...
  function calculateViewportCenter(viewport) {
    ...
    return {
      x:
        ...
        getPageScrollLeft(),
      y:
        viewportPosition.top + viewport.originalHeight / 2 - getPageScrollTop(),
    };
  }
  ...
  var InteractionObserver = /*#__PURE__*/ (function (_AbstractObserver) {
    ...
    function InteractionObserver(target) {
      ...
      _this._upHandler = _this._upHandler.bind(_assertThisInitialized(_this));
          ...
          if (event.targetTouches.length === 2) {
            ...
            if (direction !== 0) {
              if (this.fingersHypot !== null || direction === 1) {
                ...
                var clientX =
                  Math.min(pageX1, pageX2) + Math.abs(pageX1 - pageX2) / 2;
                var clientY =
                  Math.min(pageY1, pageY2) + Math.abs(pageY1 - pageY2) / 2;
                event.data = _objectSpread2(
                  _objectSpread2({}, event.data || {}),
                  {},
                  {
    ...
    _init: function _init() {
      ...
      if (options.zoomOnClick || options.zoomOnDblClick) {
        ...
        interactionObserver.on(eventType, function (event) {
          var scale =
            _this.direction === 1 ? content.maxScale : content.minScale;
          _this._computePosition(
            scale,
            eventClientX(event),
            eventClientY(event)
